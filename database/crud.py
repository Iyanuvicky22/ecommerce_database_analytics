...
def to_float(item) -> float:
    ...
    """
    return float(item) if isinstance(item, Decimal) else item
...
def customer_insights(session) -> dict:
    ...
    customer_web_device = (
        ...
        .filter(CustomersTable.device_type.ilike("Web"))
        .count()
    )
    customer_mobile_device = (
        session.query(CustomersTable)
        .filter(CustomersTable.device_type.ilike("Mobile"))
        .count()
    )
    customer_login_member = (
        session.query(CustomersTable)
        .filter(CustomersTable.login_type.ilike("Member"))
        .count()
    )
    customer_login_guest = (
        session.query(CustomersTable)
        .filter(CustomersTable.login_type.ilike("Guest"))
        .count()
    )
...
def top_product_by_sales(session) -> list:
    ...
    """
    top_5_product_by_sales = (
        session.query(
            Products.product_name, func.sum(OrderItemsTable.sales).label("total_sales")
        )
    ...
    summary = [
        {column: to_float(value) for column, value in row._mapping.items()}
...
def top_product_categories_by_revenue(session) -> list:
    ...
    top_3_product_category_by_revenue = (
        session.query(
            subquery.c.product_category,
            func.sum(subquery.c.total_sales).label("total_category_sales"),
        )
        .group_by(subquery.c.product_category)
        .order_by(desc("total_category_sales"))
        .limit(3)
    )
    summary = [
        {column: to_float(value) for column, value in row._mapping.items()}
...
def avg_order_size(session) -> float:
    ...
    subquery = (
        session.query(
            ...
            func.avg(OrderItemsTable.quantity).label("average_quantity"),
        ).group_by(OrderItemsTable.order_id)
    ).subquery()
